export class LanguageResourceKey {
  static readonly ADD_AUDIO_HYMNS = 'add_audio_hymn';
  static readonly EMPTY_STATE_DESCRIPTION =
    'audio_hymns_empty_state_description';
  static readonly FILTER_AUDIO_MSG = 'filter_audio_msg';
  static readonly ENGLISH_AUDIO_HYMNS = 'english_audio_hymns';
  static readonly EGUN_AUDIO_HYMNS = 'egun_audio_hymns';
  static readonly FRENCH_AUDIO_HYMNS = 'french_audio_hymns';
  static readonly YORUBA_AUDIO_HYMNS = 'yoruba_audio_hymns';
  static readonly UPLOAD_AUDIO_HYMN = 'upload_audio_hymn';
  static readonly UPDATE_AUDIO_HYMN = 'update_audio_hymn';
  static readonly AUDIO_HYMN_REQUIRED_MSG = 'audio_hymn_required_msg';
}
